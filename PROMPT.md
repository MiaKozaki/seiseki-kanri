# 添削工数管理システム — ゼロから作成するプロンプト

以下の仕様に従って「添削工数管理システム」をゼロから作成してください。
...
- **corrector**（添削者）: 自分の工数登録・担当業務の確認・入力作業

### 初期テストアカウント（パスワード共通: `password`）
| 名前 | メール | ロール | 担当科目 |
|------|--------|--------|----------|
| リーダー田中 | leader@test.com | leader | — |
| 山田 太郎 | yamada@test.com | corrector | 国語, 算数 |
| 鈴木 花子 | suzuki@test.com | corrector | 算数, 理科 |
| 佐藤 一郎 | sato@test.com | corrector | 国語, 社会 |
| 田中 美咲 | mtanaka@test.com | corrector | 理科, 社会, 国語 |
...
- 評価基準: 添削精度(max5), 処理速度(max5), 経験年数(max5)
- 各添削者に評価スコアを事前登録
- タスク4件（未割当状態）
- 各添削者にキャパシティ（工数）を事前登録
...
### タブ1: 概要
- KPIカード6枚: 添削者数、タスク総数、未割当タスク、完了タスク、登録工数合計、割当工数合計
...
- **日別キャパシティ推移グラフ**（ComposedChart）: 利用可能工数(棒) vs 必要作業工数(線) vs ゼロライン(ReferenceLine)
  - 各日のツールチップに余剰/不足を表示
  - 不足日数のアラート表示
- **添削者別工数テーブル**: 各添削者の総工数、割当済、残り工数、利用率プログレスバー
...
### タブ4: 振り分け
- **自動振り分けボタン**: 未割当タスクを自動でベストな添削者に割り当て
- **手動振り分け**: タスク選択 → 添削者選択 → 割当
- **現在の割り当て一覧**: タスク名、担当者名、工数、ステータス、取り消しボタン

#### 自動振り分けアルゴリズム
1. 各未割当タスクに対して、担当科目が一致する添削者を抽出
2. 各添削者のスコアを計算: `評価スコア正規化 × 0.7 + 工数マッチスコア × 0.3`
...
### タブ5: 評価管理
- 添削者ごとに各評価基準のスコアをスター（1〜max）で入力
- メモ欄付き
- リアルタイム保存

### タブ6: 作業者管理
- 添削者一覧（名前、メール、担当科目、工数サマリー）
- 新規添削者追加フォーム（名前、メール、パスワード、担当科目マルチ選択）
- 削除ボタン（関連データも連鎖削除）
